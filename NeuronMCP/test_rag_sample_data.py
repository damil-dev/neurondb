...
Works through: ClaudeDesktop -> NeuronMCP -> NeuronDB

This script demonstrates:
1. Creating sample documents table using MCP tools
2. Inserting sample data using MCP tools  
3. Generating embeddings using NeuronMCP batch_embedding tool
4. Testing RAG retrieval using retrieve_context tool

Usage:
    python test_rag_sample_data.py [--config neuronmcp_server.json]
"""
...
def main():
    ...
    parser.add_argument(
        "-c", "--config",
        default=str(Path(__file__).parent / "neuronmcp_server.json"),
