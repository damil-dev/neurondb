...
/*
 * subvector - Alias for vector_slice for compatibility
 * Extracts a subvector from start index to end index
 */
PG_FUNCTION_INFO_V1(subvector);
Datum
subvector(PG_FUNCTION_ARGS)
{
	return vector_slice(fcinfo);
}
...
vector_min_2arg(PG_FUNCTION_ARGS)
{
	...
	if (PG_NARGS() != 2)
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("neurondb: vector_min_elementwise requires 2 arguments")));
...
vector_max_2arg(PG_FUNCTION_ARGS)
{
	...
	if (PG_NARGS() != 2)
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("neurondb: vector_max_elementwise requires 2 arguments")));
