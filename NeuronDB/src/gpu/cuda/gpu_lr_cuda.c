				  ...
				  char **errstr)
{
	...
	if (n_samples > 0 && feature_dim > 0)
	{
		...
		{
			if (errstr)
				*errstr = psprintf("ndb_cuda_lr_train: feature_bytes calculation would overflow (n_samples=%d, feature_dim=%d)",
								   n_samples, feature_dim);
			pfree(weights);
			pfree(grad_weights);
			return -1;
	...
	{
		if (errstr)
			*errstr = psprintf("ndb_cuda_lr_train: label_bytes calculation would overflow (n_samples=%d)", n_samples);
		pfree(weights);
		pfree(grad_weights);
		return -1;
	}
					 ...
					 char **errstr)
{
	...
	if (n_samples > 0 && feature_dim > 0)
	{
		...
		{
			if (errstr)
				*errstr = psprintf("ndb_cuda_lr_evaluate: feature_bytes calculation would overflow (n_samples=%d, feature_dim=%d)",
								   n_samples, feature_dim);
			return -1;
		}
	}
